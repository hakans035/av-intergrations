...
import { ThankYouScreen as ThankYouScreenType } from '@/lib/types';
    ...
    if (redirectState === 'failed') {
      return (
        <div className="flex flex-col items-center justify-center text-center max-w-2xl mx-auto px-4">
          <div className="animate-fade-in-up opacity-0 w-20 h-20 rounded-full bg-orange-500/20 flex items-center justify-center mb-8">
            <AlertCircle className="w-10 h-10 text-orange-400" strokeWidth={1.5} />
          </div>
          <h2 className="animate-fade-in-up opacity-0 animation-delay-100 text-2xl md:text-3xl font-semibold mb-4 gradient-text">
            Doorsturen niet gelukt
          </h2>
          <p className="animate-fade-in-up opacity-0 animation-delay-200 text-lg text-blue-100/70 mb-8">
          ...
          <button
            ...
            className="
              ...
              group flex items-center gap-3 px-8 py-4 bg-white text-[#1062eb] rounded-2xl
              text-lg font-bold
    ...
    <div className="flex flex-col items-center justify-center text-center max-w-2xl mx-auto px-4">
      ...
      <div className="animate-fade-in-up opacity-0 mb-8">
        {isSuccess ? (
          <div className="relative">
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-green-400/30 to-emerald-600/30 flex items-center justify-center animate-float">
              <PartyPopper className="w-12 h-12 text-green-400" strokeWidth={1.5} />
            </div>
          ...
          <div className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-400/20 to-blue-600/20 flex items-center justify-center">
            <CheckCircle2 className="w-12 h-12 text-blue-300" strokeWidth={1.5} />
          </div>
        )}
      </div>

      {/* Title */}
      <h1 className="animate-fade-in-up opacity-0 animation-delay-100 text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight leading-tight mb-6">
        <span className="gradient-text">{screen.title}</span>
      </h1>

      {/* Description */}
      {screen.properties.description && (
        <p className="animate-fade-in-up opacity-0 animation-delay-200 text-lg md:text-xl text-blue-100/70 font-light mb-10 whitespace-pre-line max-w-lg leading-relaxed">
          {screen.properties.description}
        </p>
      )}
      ...
      {screen.properties.show_button && screen.properties.button_text && (
        <a
          ...
          className="
            animate-fade-in-up opacity-0 animation-delay-300
            group flex items-center gap-3 px-10 py-5 bg-white text-[#1062eb] rounded-2xl
            text-lg font-bold
            transition-all duration-300 ease-out
            shadow-[0_20px_50px_-12px_rgba(0,0,0,0.35)]
            hover:shadow-[0_25px_60px_-12px_rgba(0,0,0,0.45)]
