...
export const YesNo = memo(function YesNo({ value, onChange, onSubmit, showKeyHints = true }: YesNoProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 animate-fade-in" role="radiogroup" aria-label="Ja of Nee">
      {options.map((option, index) => {
        ...
        return (
          <button
            ...
            className={`
              flex-1 flex items-center justify-center gap-4 px-8 py-5 rounded-2xl
              ...
              ${
                isSelected
                  ...
                  : 'glass border border-white/10 hover:border-white/30 hover:bg-white/10'
              }
            `}
          >
            {/* Icon */}
            ...
            <div className="flex flex-col items-start">
              {/* Label */}
              <span className={`text-xl md:text-2xl transition-colors duration-300 ${isSelected ? 'text-white font-semibold' : 'text-white/90 font-medium'}`}>
                {option.label}
              </span>
