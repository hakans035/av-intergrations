...
import { FormDefinition, FormAnswers, FormField } from '@/lib/types';
...
export function Form({ form, onSubmit }: FormProps) {
  ...
  return (
    <div className="bg-[#1062eb] text-white min-h-screen flex flex-col antialiased relative overflow-hidden">
      ...
      {/* Navigation Footer */}
      {state === 'questions' && (
        <footer className="fixed bottom-0 left-0 right-0 p-4 md:p-6 bg-gradient-to-t from-[#1062eb] via-[#1062eb]/80 to-transparent pt-16">
          <div className="max-w-2xl mx-auto">
            <div className="glass rounded-2xl p-4 flex items-center justify-between shadow-xl shadow-black/10">
              {/* OK Button */}
              <button
                onClick={goToNext}
                className="
                  flex items-center gap-2 px-6 py-3 bg-white text-[#1062eb] rounded-xl
                  font-bold text-sm
              ...
              <div className="flex items-center gap-4">
                <div className="hidden md:flex items-center gap-2 text-white/60 text-sm">
                  ...
                  <span>~{Math.max(1, Math.ceil((form.fields.length - currentFieldIndex) * 0.15))} min</span>
                </div>
                ...
                <div className="flex items-center gap-1 glass rounded-xl p-1">
                  <button
                    ...
                    className="
                      p-2.5 rounded-lg hover:bg-white/10 transition-all duration-200
